...
| 2 | **Licence revoke check** (refunds) | Helm → Chapbook | Helm publishes a signed `revoked.json` at `https://inayatpanda.com/licences/revoked.json`; the shipped gate fetches it best-effort and rejects a revoked `jti` | **Fails open** — unreachable/404/timeout ⇒ key still works |
...
> **✅ Wired into the shipped app** (was signature + expiry only). The inline gate in
> `src/index.html` (`licenceGate` / `submitLicence`, ~line 18660) now performs the
> revocation check after the signature + expiry pass. It **fails open**, so a refunded
> key is rejected once Helm's list reaches the browser, but a transient network problem
...
The gate fetches this **best-effort**: at most once per 24 h, with a 4 s timeout, and
only a successfully fetched **and Ed25519 signature-valid** list (verified with the same
key as the licence keys) ever updates the `localStorage` cache (a tampered host cannot
poison it), with a monotonic guard so a validly signed but older list never replaces a
newer cached one. It **fails open** — unreachable / 404 / malformed / timeout ⇒ fall
back to the last cached list; never fetched ⇒ the key is allowed. A revoked `jti` locks
the app with *"This licence has been revoked."*

**Timing.** A *newly entered* key awaits the revocation check before it is accepted. A
*stored* key boots immediately (boot is never blocked on the network): a
previously-cached revocation locks it synchronously, otherwise the list is refreshed in
the background and the app hard re-gates if the key is now revoked.

The pure predicates (`isRevoked`, `shouldReplaceCache`) live unit-tested in
`src/lib/revocation.js`; the inline gate **mirrors** them verbatim (the inline module
cannot `import` — `src/lib/` is not shipped to `dist/`), the same pattern used for
`AI_DEFAULT_MODELS`. The Helm-generated `licence-client.js` template
(`src/lib/templates/licence-client.js`, with its `__REVOKED_URL__` placeholder) remains
the canonical reference this logic was mirrored from.
